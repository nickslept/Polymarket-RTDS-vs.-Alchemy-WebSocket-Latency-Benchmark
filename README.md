...
- **Polymarket RTDS**: Polymarket's Real-Time Data Streaming WebSocket service (`wss://ws-live-data.polymarket.com`), which streams trade activity directly. No authentication required.
- **Alchemy**: a blockchain infrastructure provider that streams the on-chain `OrderFilled` logs emitted by Polymarket's exchange contracts on the Polygon blockchain via WebSocket. Requires an Alchemy API key.
...
- **Figuring out the fastest free source of data** for same-trade detection to inform a trading system.
...
```mermaid
flowchart LR
    P["Polymarket RTDS WS<br/>tx_hash + arrival time (ns)"] --> H{"Hashmap<br/>automatic matching by tx_hash"}
    A["Alchemy WS<br/>tx_hash + arrival time (ns)"] --> H
...
4. **Unmatched trades become "orphans."** A trade can fail to match for two reasons:
   - **`unmatched`**: only one source ever reported it within the maximum time window (default 60s; editable in config), so it's evicted by a periodic sweep.
...
5. **Trades are only stored in the hashmap when both connections are up.** If either connection drops, then collection pauses, the hashmap is cleared, and the tool reconnects with exponential backoff capped at a ceiling (default 30s; editable in config). Once both connections are live again, the reconnection delay resets back to the base (default 1s; editable in config). This prevents the hashmap from filling with trades that could never be matched.
...
| ---------------- | ------- | ----------------------------------------------------------------------- |
| `tx_hash`        | string  | Transaction hash on the Polygon blockchain.                                |
| `poly_rel_ns`    | int64   | Polymarket RTDS trade arrival time, in ns relative to run start.              |
| `alchemy_rel_ns` | int64   | Alchemy trade arrival time, in ns relative to run start.                      |
...
- The magnitude is the gap in **nanoseconds**.
...
Clone the repository (requires [Git](https://git-scm.com/)). Then `cd` into the project root. All remaining commands are run from there.
...
| `MATCH_TIMEOUT_SECONDS`     | `60`    | How long a trade waits for a match in the hashmap before it's evicted as an orphan. |
| `EVICTION_INTERVAL_SECONDS` | `30`    | How often the hashmap is checked for old unmatched trades (for removal).           |
...
These are the current **Polymarket V2 exchange contracts on Polygon mainnet** (Chain ID 137) and the `OrderFilled` event topic, pulled from PolygonScan. Polymarket migrated from V1 to these V2 contracts in 2026, so the addresses *can* change again. If this happens, update these values here and the Alchemy listener *should* continue working as intended (unless Alchemy changes its WebSocket endpoint).

---

## Roadmap

Future direction (this tool is a proof of concept):

- **Multi-RPC-node support**: extend the benchmark to include multiple RPC providers / nodes to gather more data.
